...
function renderCopy(slide, language, index, total) {
  const footer = language === "zh" ? "VoltStream · 仅使用合成/公开数据" : "VoltStream · synthetic/public data only";
...
function renderBody(slide, language) {
  ...
  switch (slide.layout) {
    ...
    case "fact":
      return `<div class="fact-box"><div class="fact-label">${isChinese ? "意外发现" : "Surprising fact"}</div><div><p class="fact-copy">${escapeHtml(slide.fact)}</p><p class="implication large-claim"><span class="accent">${isChinese ? "设计含义：" : "Design implication:"}</span> ${escapeHtml(slide.implication)}</p><p class="source-line">${escapeHtml(slide.sources)}</p></div></div>`;
    case "choices":
    ...
    case "case":
      return `<div class="conflict"><div class="source-value"><span>${escapeHtml(slide.sourceValues[0].label)}</span><strong>${escapeHtml(slide.sourceValues[0].value)}</strong></div><div class="versus">≠</div><div class="source-value"><span>${escapeHtml(slide.sourceValues[1].label)}</span><strong>${escapeHtml(slide.sourceValues[1].value)}</strong></div></div><div class="canonical-answer"><span>${isChinese ? "标准字段：" : "Canonical field:"} <strong>${escapeHtml(slide.canonical)}</strong></span><br><span>${isChinese ? "安全答案：" : "Safe answer:"} <strong>${escapeHtml(slide.safeAnswer)}</strong></span></div><table class="outcome-table"><tbody>${slide.outcomes.map(outcome => `<tr><td>${escapeHtml(outcome.strategy)}</td><td>${escapeHtml(outcome.result)}</td><td class="${outcome.status.includes("unsafe") ? "bad" : outcome.status === "correct" ? "good" : "warn"}">${escapeHtml(outcome.statusLabel || outcome.status)}</td></tr>`).join("")}</tbody></table><p class="note-line">${escapeHtml(slide.takeaway)}</p>`;
    case "signal":
      return `<div class="signal-grid">${slide.signals.map(signal => `<article class="signal"><h3>${escapeHtml(signal.title)}</h3><p class="signal-value">${escapeHtml(signal.value)}</p><p class="muted">${escapeHtml(signal.description)}</p></article>`).join("")}</div><p class="next-step"><span class="accent">${isChinese ? "下一轮测试：" : "Next test:"}</span> ${escapeHtml(slide.next)}</p>`;
    case "testing":
